...
param_grid = {
    'C': [0.001, 0.01, 0.1, 1],
    'penalty': ['l1', 'l2'],  # L1 和 L2 正则化
    'solver': ['liblinear']  # 选择适用于小数据集和二分类问题的求解器
}
...
grid_search = GridSearchCV(logreg, param_grid, refit=True, verbose=2, cv=5)
...
# 保存模型到文件
model_filename = './pkl/logistic_regression_model-929-all.pkl'
joblib.dump(best_logreg, model_filename)
print(f"Best model saved to {model_filename}")

# 保存标准化器
scaler_filename = './pkl/logreg-scaler-929-all.pkl'
...
plot_learning_curve(best_logreg, X_train_scaled, y_train, cv=5)
plt.xticks(fontsize=25)  # 增加x轴刻度字体大小
plt.yticks(fontsize=25)  # 增加y轴刻度字体大小

plt.tight_layout()  # 自动调整子图参数
plt.savefig('./png/LR-new.png', dpi=400)
plt.show()


# 绘制 ROC 曲线
y_score = best_logreg.predict_proba(X_test_scaled)[:, 1]  # 获取正类的概率
