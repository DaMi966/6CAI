...
grid_search = GridSearchCV(knn, param_grid, refit=True, verbose=2, cv=5)
...
plot_learning_curve(best_knn, X_train_scaled, y_train, cv=5)
plt.xticks(fontsize=25)  # 增加x轴刻度字体大小
plt.yticks(fontsize=25)  # 增加y轴刻度字体大小

plt.tight_layout()  # 自动调整子图参数
plt.savefig('./png/KNN-new.png', dpi=400)
plt.show()


# 绘制 ROC 曲线
y_score = best_knn.predict_proba(X_test_scaled)[:, 1]  # 获取正类的概率
...
# 保存模型到文件
model_filename = './pkl/knn_new_model-1011.pkl'
joblib.dump(best_knn, model_filename)
print(f"Best model saved to {model_filename}")

# 保存标准化器
scaler_filename = './pkl/knn_new_scaler-1011.pkl'
